...
class TestModelLoading(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # ✅ Load vectorizer with proper exception handling
        try:
    ...
    def get_latest_model_version(model_name):
        """Retrieve the latest model version from MLflow."""
        client = mlflow.MlflowClient()
        latest_versions = client.search_model_versions(f"name='{model_name}'")

        if not latest_versions:
            logging.error(f"No registered versions found for model: {model_name}")
            return None  # No model found

        # ✅ Ensure we fetch the **highest** version number
        latest_version = max(latest_versions, key=lambda v: int(v.version))
        logging.info(f"Latest model version found: {latest_version.version}")
