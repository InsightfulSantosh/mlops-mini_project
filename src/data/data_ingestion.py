...
def processing(df):
    ...
    try:
        ...
        processed_df = (
            ...
            .assign(
                sentiment=lambda x: x["sentiment"].map({"happiness": 1, "sadness": 0})
            )
            .sample(frac=1, random_state=42)  # Shuffle the data
        )
        logger.info(
            f"Data processed successfully. {processed_df.shape[0]} rows remaining."
        )
        return processed_df
...
def main():
    try:
        ...
        try:
            test_size = params.get("data_ingestion", {}).get(
                "test_size", 0.2
